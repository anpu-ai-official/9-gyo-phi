...
A private listening library for PDFs, notes, articles, and technical writing. Import a document, choose a voice, and pick up where you left off. No account or subscription is required.
...
The app does **not** need Python. macOS system voices work locally and support WAV export. A separate browser preview has its own library; use a backup to transfer documents into the desktop app.
...
2. Try one of three original short reads, import a PDF/TXT/Markdown file, or start in **Text studio**.
...
- A PDF reader with page navigation, fit-to-width rendering, zoom, highlighted passages, and a searchable transcript. The text reader renders a bounded window of passages for large documents.
...
| Local MLX | Start the optional Python engine | App connects to `127.0.0.1:8765`; model downloads come from Hugging Face | Yes |
...
- Text: UTF-8 TXT/Markdown or pasted text, 1–100,000 characters. Markdown is read as text.
...
Python 3.11+ and Apple Silicon are required for this optional path. Use a dedicated environment; the desktop UI never auto-starts or searches for a host Python installation.
...
The engine retains its PDF, audio, and technical verbalization APIs. Browser code transformations use deterministic local rules; model-specific verbalization is available through `/api/verbalize`. The catalog does not provide arbitrary repository downloads or a model-switching UI.
...
src-tauri/src/lib.rs           Native macOS voices and background speech tasks
